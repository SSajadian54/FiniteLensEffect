...
period=float(5.0*24.0*3600.0)#[s]
...
for i in range(n1): 
    for j in range(n2): 
        ...
        print ("nx, ny, dx, dy, rho:  ",  nx, ny, dx, dy,   Rho, Mwd,   Rwd, i, j )
        if(Rwd<Rin): flag=0;  
        else:        flag=1; 
        if(abs(Rwd/RE-1.415)<0.01): 
            thrx.append(i)
            thry.append(j)
        ...
        print(Rstar, Rstarp,  RE, semi/AU,  Amax,  Astar1,  Astar2,  Fbase, Rwd/Rin)
        print("*************************************************************")
        Magni[j,i]=Amax
        ...
        np.savetxt(fil0,par.reshape((-1,8)),fmt ="%d  %d  %.4f  %.4f  %.7f  %.7f  %.7f  %.7f")
        fil0.close();
...
cbar.set_label(r"$\Delta A_{\rm{max}}(\times 100)$", rotation=270, fontsize=21, labelpad=20.0)
...
fig.savefig("Amax{0:d}_{1:d}.jpg".format(int(period/24.0/3600.0), int(Dl*1000.0/KPC)), dpi=200)     
...
fig=plt.gcf()
fig.tight_layout(pad=0.15)
fig.savefig("Rinw{0:d}_{1:d}.jpg".format(int(period/24.0/3600.0), int(Dl*1000.0/KPC)), dpi=200)     
'''    
################################################################################
#### Delat 1
...
cbar=plt.colorbar(orientation='vertical',shrink=0.9,pad=0.01, ticks=v)
cbar.ax.tick_params(labelsize=17)
cbar.set_label(r"$\Delta_{1}(\times 100)$", rotation=270, fontsize=21, labelpad=20.0)
...
fig=plt.gcf()
fig.tight_layout(pad=0.15)
fig.savefig("DeltaA{0:d}_{1:d}.jpg".format(int(period/24.0/3600.0), int(Dl*1000.0/KPC)), dpi=200)     
...
fig=plt.figure(figsize=(7,6))
ax= plt.gca()  
plt.imshow(Delta2,cmap=cmap,interpolation='nearest',aspect='equal', origin='lower')
#plt.plot(thrx, thry, "m*", markersize=3)
#plt.scatter(Flag,cmap=discrete_cmap(n1*n2, 'cubehelix'),interpolation='nearest',aspect='equal', origin='lower')
plt.clim()
minn=np.min(Delta2)
...
cbar=plt.colorbar(orientation='vertical',shrink=0.9,pad=0.01, ticks=v)
cbar.ax.tick_params(labelsize=17)
cbar.set_label(r"$\Delta_2(\times 100)$", rotation=270, fontsize=21, labelpad=20.0)
#contours=plt.contour(Delta2, 10,  colors='black')
#plt.clabel(contours, inline=5, fontsize=17)
plt.title( r"$T(\rm{days})=$"+str(int(period/24.0/3600.0)),fontsize=21)    
plt.xticks(fontsize=20, rotation=0)
plt.yticks(fontsize=20, rotation=0)
...
fig=plt.gcf()
fig.tight_layout(pad=0.15)
fig.savefig("DeltaB{0:d}_{1:d}.jpg".format(int(period/24.0/3600.0),int(Dl*1000.0/KPC)), dpi=200)     
...
fig=plt.figure(figsize=(7,6))
ax= plt.gca()  
plt.imshow(Delta3,cmap=cmap,interpolation='nearest',aspect='equal', origin='lower')
#plt.plot(thrx, thry, "m*", markersize=3)
#plt.scatter(Flag,cmap=discrete_cmap(n1*n2, 'cubehelix'),interpolation='nearest',aspect='equal', origin='lower')
plt.clim()
minn=np.min(Delta3)
...
cbar=plt.colorbar(orientation='vertical',shrink=0.9,pad=0.01, ticks=v)
cbar.ax.tick_params(labelsize=17)
cbar.set_label(r"$\Delta_3 (\times 100)$", rotation=270, fontsize=21, labelpad=20.0)
#contours=plt.contour(Delta3, 10,  colors='black')
#plt.clabel(contours, inline=5, fontsize=17)
plt.title( r"$T(\rm{days})=$"+str(int(period/24.0/3600.0)),fontsize=21)    
plt.xticks(fontsize=20, rotation=0)
plt.yticks(fontsize=20, rotation=0)
...
fig=plt.gcf()
fig.tight_layout(pad=0.15)
fig.savefig("DeltaC{0:d}_{1:d}.jpg".format(int(period/24.0/3600.0), int(Dl*1000.0/KPC)), dpi=200)     
